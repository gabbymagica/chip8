...
void reset_cursor() { printf("\033[H\033[J"); }
...
int main() {
  ...
  for (;;) {
    // fetch
    // pega os dois bytes na memória, que equivalem a 2*8 = 16 bits, um opcode
    // hexadecimal
    ...
    */
    ...
    case 0x8:
      ...
      case 0xE: {
        ...
        // Set VF to 1 if the bit that was shifted out was 1, or 0 if it was 0
        uint8_t x = (opcode & 0x0F00) >> 8;
        // pega o primeiro dígito (já que ele sempre tem 8bits, só fazer um and
        // com 1000 0000)
    ...
    case 0xC: {
      ...
      // value NN, and puts the result in VX. pega o X -> (opcode & 0x0F00) >> 8
      // pega o NN -> (opcode & 0x00FF)
    ...
    case 0xD:
      // display
      registers[15] = draw_gfx(gfx, registers[(opcode & 0xF00) >> 8],
                               registers[(opcode & 0xF0) >> 4], (opcode & 0xF),
    ...
    case 0xF:
      ...
      case 0x9:
        index_register =
            memory[FONTSET_START_ADDRESS + (((opcode & 0x0F00) >> 8) * 5)];
        break;
      case 0x3: {
